...
An advanced, tool-enabled AI diagnostic agent designed to run inside Google Colab. The system uses Function Calling via the Groq API (with an OpenAI failover fallback layer) to dynamically query, sort, and isolate structural errors or text patterns across any target system or application log repository.
🚀 Key Features
...
Before executing the script, map your private API tokens within your Google Colab workspace:
    ...
    Click on the Key Icon (Secrets) in the left-hand sidebar menu.
    Add a new secret named GROQ_API_KEY and insert your Groq API key.
    Add an optional secret named OPENAI_API_KEY to act as your failover ecosystem layer.
    Toggle Notebook Access to "On" for both keys.
